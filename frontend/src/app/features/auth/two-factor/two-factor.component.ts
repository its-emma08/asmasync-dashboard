...
@Component({
    ...
    <div class="w-full flex justify-center">
      <div class="w-full max-w-md glass-login-card p-8 md:p-12 animate-fade-in-up border border-slate-100 dark:border-slate-700 relative z-10">
        
        <div class="text-center mb-8">
            <div class="w-24 h-24 bg-cyan-50 dark:bg-cyan-500/10 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-cyan-100 dark:border-cyan-500/20">
                <mat-icon class="text-brand-cyan scale-150 transform" style="font-size: 40px; width: 40px; height: 40px;">lock</mat-icon>
            </div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-2">Verificación de Dos Pasos</h2>
            <p class="text-slate-500 dark:text-slate-400">Hemos enviado un código al correo de tu cuenta.</p>
        </div>

        <form [formGroup]="otpForm" (ngSubmit)="onSubmit()" class="flex flex-col gap-6">
            <div class="space-y-2">
                <label class="font-bold text-slate-700 dark:text-slate-300 ml-1">Código de Seguridad</label>
                <input 
                    type="text" 
                    formControlName="code" 
                    maxlength="6"
                    class="w-full text-center text-2xl tracking-[0.5em] font-bold py-4 bg-slate-50 dark:bg-slate-800/60 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-brand-cyan/20 focus:border-brand-cyan outline-none transition-all placeholder-slate-300 dark:placeholder-slate-500"
                    placeholder="000000"
                >
